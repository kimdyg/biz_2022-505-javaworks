...
public class Var_03 {
	...
	public static void main(String[] args) {
		...
		//bEven이 true 이면 괄호안을 연산하라
		//if 조건명령문
		/*
		 * 비교(조건) 명령문
		 * if(boolean) {boolean이 true 일때 실행  }
		 * 		else { boolean 이 true가 아닌때 실행 }
		 * boolean 의 값이 true 인가 false 인가에 따라서
		 * 명령문을 선택적으로 실행할 수 있다.
		 */
		if (bEven) {
				System.out.println(intNum1 + " (은/는) 짝수 ");
			} else System.out.println(intNum1 + " (은/는) 짝수가 아니다");
		...
		if ((intNum1 % 4) == 0) {
			...
			System.out.println(intNum1 + " 그런데 2의 배수인 것 같음");
		}
